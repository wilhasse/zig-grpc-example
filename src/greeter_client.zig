...
pub fn main() !void {
    ...
    const target = try fmt.bufPrint(&tb, "0.0.0.0:{d}", .{args.port});
    ...
    while (i <= 10000) : (i += 1) {
        var nb = [_]u8{0} ** 15;
        const user = try fmt.bufPrint(&nb, "Elfo #{d}", .{i}); // hi, im Elfo!
        const response: [*c]const u8 = c_greeter.greeter_client_say_hello(&greeter_client.?.*, user.ptr);
